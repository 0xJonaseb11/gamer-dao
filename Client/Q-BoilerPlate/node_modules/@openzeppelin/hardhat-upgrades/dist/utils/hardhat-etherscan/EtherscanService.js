...
async function verifyContract(url, req) {
    ...
    try {
        ...
        throw new upgrades_core_1.UpgradesError(`Failed to send contract verification request.
Endpoint URL: ${url}
...
try to wait for five confirmations of your contract deployment transaction before running the verification subtask.`);
    ...
    if (!etherscanResponse.isOk()) {
        throw new upgrades_core_1.UpgradesError(etherscanResponse.message);
    }
...
async function getVerificationStatus(url, req) {
    ...
    try {
        ...
        if (!(response.statusCode >= 200 && response.statusCode <= 299)) {
            ...
            throw new upgrades_core_1.UpgradesError(message);
        }
    }
    catch (error) {
        throw new upgrades_core_1.UpgradesError(`Failure during etherscan status polling. The verification may still succeed but
...
Reason: ${error.message}`);
    ...
    if (!etherscanResponse.isOk()) {
        throw new upgrades_core_1.UpgradesError(`The Etherscan API responded with a failure status.
